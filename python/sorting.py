...
def quick_sort(items):
  ...
  if items == []:
      ...
      # recursion: concat sorted list smaller elements, pivot and sorted list of larger elements
      pivot = items[0]
      return quick_sort(filter(lambda x:x <= pivot, items[1:])) + [pivot] + quick_sort(filter(lambda x:x > pivot, items[1:]))
